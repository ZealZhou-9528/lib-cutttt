...
void gAdd_25D(vector<double>& coordnew, vector< vector<double> >& gcoord, int LineNode[2], double CircleProperty[4], double Cycle, double Magnitude, int ni);
...
void CutEleWithFun_25D(vector< vector<double> >& gcoord, vector< vector<int> >& nodes, vector<int> CrossElement, double CircleProperty[4], double Cycle, double Magnitude, int ni, double CrossSectExp, double p, double q)
{
	...
	LevSetFunVal_25D(gcoord, CircleProperty, phi, Cycle, Magnitude, ni, CrossSectExp, p, q);
	int LocalNode[6][2] = { 0,1,0,2,0,3,1,2,1,3,2,3 };
	int crl = 0;   //CroEleNodes行数
	//crl = _msize(CrossElement) / sizeof(int);
...
#pragma omp parallel for
	...
	//vector<vector<int>> EdgeNodes(crl, vector<int>(2, 0));
	double** NodesFai = new double* [crl];    //定义NodesFai   496*2
	for (int i = 0; i < crl; i++)
		NodesFai[i] = new double[2]();
	//vector<vector<int>> NodesFai(crl, vector<int>(2, 0));
	double* DotValues = new double[crl]();    //定义DotValues  496*1
	//vector<double> DotValues(crl, 0);
	...
	for (int aen = 0; aen < CENn; aen++)
	{
		...
		gAdd_25D(coordnew, gcoord, LineNode, CircleProperty, Cycle, Magnitude, ni);
		//gcoord = [gcoord;coord]; 
