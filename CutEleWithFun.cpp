...
void CutEleWithFun(vector< vector<double> >&gcoord, vector< vector<int> >&nodes, vector<int> CrossElement, double CircleProperty[4], double Cycle, double Magnitude,int ni, double CrossSectExp, double p, double q)
{
	...
	LevSetFunVal(gcoord, CircleProperty, phi, Cycle, Magnitude, ni, CrossSectExp, p, q);
	...
	for (int i = 0; i < AENw; i++)
		AENnew[i] = new int[2]();
	//vector< vector<int> > AENnew(AENw, vector<int>(2, 0));
	...
	for (int aen = 0; aen < CENn; aen++)
	{
		...
		gAdd(coordnew,gcoord,LineNode, CircleProperty,Cycle,Magnitude,ni);
		//gcoord = [gcoord;coord]; 
	...
	//for (int i = 0; i < AENn; i++)   //删除AllEdgNodes数组
	//	delete[]AllEdgNodes[i];
	//delete[]AllEdgNodes;
