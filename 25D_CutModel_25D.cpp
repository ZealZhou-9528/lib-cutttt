...
void CutEleWithFun_25D(vector< vector<double> >& gcoord, vector< vector<int> >& nodes, vector<int> CrossElement, double CircleProperty[4], double Cycle, double Magnitude, int ni, double CrossSectExp, double p, double q);
...
#pragma omp parallel for
	...
	vector<vector<double>> CenterPoints(12, vector<double>(3, 0));
	CenterPoints = { {Xmax,Ymin * 0.75f ,Zmax * 1.8f / 2.025f} ,{Xmax,Ymin * 0.25f ,0},{Xmax,Ymax * 0.25f ,Zmax * 1.8f / 2.025f},{Xmax,Ymax * 0.75f ,0},
					 {Xmax,Ymin * 0.75f ,Zmax * 0.9f / 2.025f} ,{Xmax,Ymin * 0.25f ,Zmin * 0.9f / 2.025f},{Xmax,Ymax * 0.25f ,Zmax * 0.9f / 2.025f},{Xmax,Ymax * 0.75f ,Zmin * 0.9f / 2.025f},
	                 {Xmax,Ymin * 0.75f ,0} ,{Xmax,Ymin * 0.25f ,Zmin * 1.8f / 2.025f} ,{Xmax,Ymax * 0.25f ,0} ,{Xmax,Ymax * 0.75f ,Zmin * 1.8f / 2.025f} };
	vector<vector<double>> CirclesProperty(12, vector<double>(4, 0));
	ModelFunction(CenterPoints, CircumR, CirclesProperty);

	//筛选出需要被切割的单元
	vector< vector<int> >ElementStatus(nodes.size(), vector<int>(12, 0));
	CutElement_25D(ElementStatus, CirclesProperty, gcoord, nodes, CircumRint, Magnitude, CrossSectExp, p, q);
...
#pragma omp parallel for
	for (int i = 0; i < CenterPoints.size(); i++)  //此循环为了确定InnerEle，OuterEle，CrossEle三次的大小
	{
		//#pragma omp parallel for
	...
	for (int i = 0; i < CenterPoints.size(); i++)
	{
		...
		CutEleWithFun_25D(gcoord, nodes, CrossElement, CircleProperty, CircumRint, Magnitude, i, CrossSectExp, p, q);
	}

	//判断筛选之后单元所在位置
	vector<int> InnerElements;// , OuterElements, CrossElements;
//#pragma omp parallel for
	for (int i = 0; i < CenterPoints.size(); i++)  // for i = 1:size(CirclesProperty,1)
	{
		...
		CutElement_25D(CrossElesStatu, CirclesProperty, gcoord, nodes, CircumRint, Magnitude, CrossSectExp, p, q);
