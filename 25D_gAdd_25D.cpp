...
void gAdd_25D(vector<double>& coordnew, vector< vector<double> >& gcoord, int LineNode[2], double CircleProperty[4], double Cycle, double Magnitude, int ni)
{
	...
	PlaneFunction_25D(f1, x1, y1, z1, CircleProperty, Cycle, Magnitude, ni);//f1=PlaneFunction(x1,y1,z1,PlanePropery,Cycle,Magnitude,n);  
	...
	while (Distance > CovgCrit)
	{
		...
		PlaneFunction_25D(f3, x3, y3, z3, CircleProperty, Cycle, Magnitude, ni);//f3=PlaneFunction(x3,y3,z3,PlanePropery,Cycle,Magnitude,n);
		if (f3 * f1 > 0)
		{
			x1 = x3; y1 = y3; z1 = z3;
		}
